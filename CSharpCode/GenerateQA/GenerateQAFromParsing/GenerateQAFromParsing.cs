/*
 * Вспомогательная утилита для чат-бота http://raum.org.ru/chatbot
 * Генерирует датасет с тройками предложение ПРЕДПОСЫЛКА-ВОПРОС-ОТВЕТ, например как тут http://raum.org.ru/NLP_Datasets/blob/master/QA/premise_question_answer4.txt
 * 
 * Использует результаты работы утилиты http://raum.org.ru/chatbot/tree/master/CSharpCode/ExtractFactsFromParsing
 * 
 * Для морфологического разбора фактов используется API Грамматического Словаря (http://solarix.ru/api/ru/list.shtml),
 * исходные тексты на C# и C++ лежат в отдельном репозитории здесь http://raum.org.ru/GrammarEngine/tree/master/src/demo/ai/solarix/engines
 * 
 * Частеречная разметка требует наличия русской словарной базы. Можно взять готовые файлы здесь http://raum.org.ru/GrammarEngine/tree/master/src/bin-windows64 или
 * собрать ее самостоятельно из исходных текстов, которые лежат здесь http://raum.org.ru/GrammarEngine/tree/master/src/dictionary.src
 */
